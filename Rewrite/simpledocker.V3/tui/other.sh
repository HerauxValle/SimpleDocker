...
_main_menu_header() {
    ...
    if [[ -n "$IMG_PATH" ]] && mountpoint -q "$MNT_DIR" 2>/dev/null; then
        local used_kb total_bytes used_gb total_gb
    ...
    printf '%s%s' "${L[title]}" "$img_label"
}

_show_ct_log() {
    local cid="${1:-$SD_MENU_CTX}"
    local meta_log; meta_log=$(jq -r '.meta.log // empty' "$CONTAINERS_DIR/$cid/service.json" 2>/dev/null)
    local lf
    if [[ -n "$meta_log" ]]; then
        lf="$(_cpath "$cid")/$meta_log"
    else
        lf=$(_log_path "$cid" "start")
    fi
    if [[ -f "$lf" ]]; then
        sd_msg "$(tail -100 "$lf" 2>/dev/null | cat)"
    else
        sd_msg "No log yet for '$(_cname "$cid")'."
    fi
}
