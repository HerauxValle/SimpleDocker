...
_do_pkg_update() {
    ...
    [[ ! -f "$mf" ]] && { sd_msg "No manifest. Reinstall first."; return; }
    ...
    [[ -z "$dep_pkgs" && -z "$pip_pkgs" && -z "$npm_pkgs" && -z "$gh_repos" ]] && { sd_msg "Nothing to update."; return; }
    ...
    sd_sd_confirm "$(printf "Update packages for '%s'?\n\n%s" "$(_cname "$cid")" "$_um")" || return
...
_do_ubuntu_update() {
    ...
    if sd_sd_confirm "$(printf "Create a backup first?\n\n  Will appear in Backups as '%s'." "$snap_label")"; then
        ...
        if btrfs subvolume snapshot -r "$install_path" "$sdir/$snap_id" &>/dev/null \
            ...
            sd_confirm "⚠  Backup failed. Continue anyway?" || return
        fi
    fi
