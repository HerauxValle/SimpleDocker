...
_do_restore_snap() {
    ...
    sd_msg "$(printf "Restored '%s' from '%s'." "$name" "$snap_label")"
}
...
_prompt_backup_name() {
    ...
    while true; do
        local input
        if ! sd_input "$(printf 'Backup name:\n  (leave blank for random: %s)' "$default_id")"; then
            input="$default_id"
        else
            input="${SD_MSG//[^a-zA-Z0-9_\-]/}"
            [[ -z "$input" ]] && input="$default_id"
        fi
        [[ -d "$sdir/$input" ]] && { sd_msg "A backup named '$input' already exists."; continue; }
...
_create_manual_backup() {
    ...
    [[ -z "$install_path" || ! -d "$install_path" ]] && { sd_msg "No installation found for '$name'."; return 1; }
    local sdir; sdir=$(_snap_dir "$cid"); mkdir -p "$sdir" 2>/dev/null
    ...
    if ! btrfs subvolume snapshot -r "$install_path" "$sdir/$snap_id" &>/dev/null; then
        cp -a "$install_path" "$sdir/$snap_id" 2>/dev/null || { sd_msg "Snapshot failed."; return 1; }
    fi
    _snap_meta_set "$sdir" "$snap_id" "type=manual" "ts=$ts"
    sd_msg "$(printf "Backup '%s' created." "$snap_id")"
}

_clone_from_snap() {
    local src_cid="$1" snap_path="$2" snap_label="$3"
    local src_name; src_name=$(_cname "$src_cid")
    [[ ! -d "$snap_path" ]] && { sd_msg "Snapshot not found."; return 1; }
    sd_input "Name for the clone:" || return 1
    local clone_name="$SD_MSG"
    [[ -z "$clone_name" ]] && { sd_msg "No name given."; return 1; }
    ...
    if btrfs subvolume snapshot "$snap_path" "$clone_path" &>/dev/null; then
        ...
        sd_msg "$(printf "Cloned '%s' (%s) → '%s'" "$src_name" "$snap_label" "$clone_name")"
    else
        cp -a "$snap_path/." "$clone_path/" 2>/dev/null \
            || { rm -rf "$clone_dir" "$clone_path" 2>/dev/null; sd_msg "Clone failed."; return 1; }
        sd_msg "$(printf "Cloned '%s' (%s) → '%s' (plain copy)" "$src_name" "$snap_label" "$clone_name")"
    fi
}
...
_process_install_finish() {
    ...
    if [[ -f "$ok_file" ]]; then
        ...
        if [[ "$_ok_age" -gt 600 ]] && ! tmux_up "$(_inst_sess "$cid")"; then
            rm -f "$ok_file"; sd_msg "⚠  Installation result is stale. Please reinstall."; return
        fi
        rm -f "$ok_file"
        ...
        if [[ "$(_st "$cid" installed)" == "true" ]]; then
            ...
            sd_msg "$(printf "'%s' packages updated." "$name")"
            return
        fi
        # Fresh install
        ...
        if sd_sd_confirm "$(printf "'%s' ${L[msg_install_ok]}\n\nCreate a Post-Install backup?\n  (Instant revert to clean install)" "$name")"; then
            ...
            if btrfs subvolume snapshot -r "$_pi_path" "$_pi_sdir/$_pi_id" &>/dev/null; then
                ...
                sd_msg "$(printf "Backup 'Post-Installation' created for '%s'." "$name")"
            else
                cp -a "$_pi_path" "$_pi_sdir/$_pi_id" 2>/dev/null \
                    && _snap_meta_set "$_pi_sdir" "$_pi_id" "type=manual" "ts=$_pi_ts" \
                    && sd_msg "$(printf "Backup 'Post-Installation' created for '%s'." "$name")" \
                    || sd_msg "$(printf "Backup failed for '%s' — disk full?" "$name")"
            fi
        else
            sd_msg "'$name' ${L[msg_install_ok]}"
        fi
    elif [[ -f "$fail_file" ]]; then
        rm -f "$fail_file"; sd_msg "${L[msg_install_fail]}"
    fi
