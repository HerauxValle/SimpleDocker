...
_run_job() {
    ...
    if [[ "$mode" == "install" ]]; then
        [[ -z "$install_path" ]] && { sd_msg "No install path set."; return 1; }
        ...
        [[ -z "$_check" && -z "$_ghck" && -z "$_dirck" && -z "$_pipck" && -z "$_npmck" ]] && \
            { sd_msg "⚠  No install, git, dirs, pip, or npm block in service.json."; return 1; }
        ...
        [[ -z "$install_path" || ! -d "$install_path" ]] && { sd_msg "Not installed."; return 1; }
    fi
