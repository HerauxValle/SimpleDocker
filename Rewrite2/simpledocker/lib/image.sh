...
_mount_img() {
    ...
    if _img_is_luks "$1"; then
        _luks_open "$1" || { rmdir "$MNT_DIR" 2>/dev/null; sd_msg "Failed to unlock image."; return 1; }
...
_yazi_pick() {
    local filter="${1:-}"
    local tmp; tmp=$(mktemp "$TMP_DIR/.sd_tmp_XXXXXX" 2>/dev/null) || { sd_msg "mktemp failed (TMP_DIR=$TMP_DIR)"; return 1; }
...
'); rm -f "$tmp"
    ...
    if [[ -n "$filter" && "${chosen##*.}" != "$filter" ]]; then
        sd_msg "Please select a .$filter file."; return 1
    fi
    printf '%s' "${chosen%/}"
}
...
_create_img() {
    ...
    sd_input "$(printf "Image name (e.g. simpleDocker):\n\n  %b  The name cannot be changed after creation." "${RED}⚠  WARNING:${NC}")" || return 1
    name="${SD_MSG//[^a-zA-Z0-9_\-]/}"
    [[ -z "$name" ]] && { sd_msg "No name given."; return 1; }
    sd_input "Max size in GB (sparse — only uses actual disk space, leave blank for 50 GB):" || return 1
    size_gb="$SD_MSG"
    [[ -z "$size_gb" ]] && size_gb=50
    [[ ! "$size_gb" =~ ^[0-9]+$ || "$size_gb" -lt 1 ]] && { sd_msg "Invalid size."; return 1; }
    sd_pick_dir || { sd_msg "No directory selected."; return 1; }
    dir="$SD_MSG"
    imgfile="$dir/$name.img"
    [[ -f "$imgfile" ]] && { sd_msg "Already exists: $imgfile"; return 1; }
    truncate -s "${size_gb}G" "$imgfile" 2>/dev/null || { sd_msg "Failed to allocate image file."; return 1; }
    ...
    printf '%s' "$SD_VERIFICATION_CIPHER" | sudo -n cryptsetup luksFormat \
        ...
        || { rm -f "$imgfile"; sd_msg "luksFormat failed."; return 1; }
    ...
    printf '%s' "$SD_VERIFICATION_CIPHER" | sudo -n cryptsetup open --key-file=- "$imgfile" "$_mapper" &>/dev/null \
        || { rm -f "$imgfile"; sd_msg "LUKS open failed."; return 1; }

    sudo -n mkfs.btrfs -q -f "/dev/mapper/$_mapper" &>/dev/null \
        || { sudo -n cryptsetup close "$_mapper"; rm -f "$imgfile"; sd_msg "mkfs.btrfs failed."; return 1; }
    ...
    _enc_authkey_create "$_tf_img_auth" || { rm -f "$_tf_img_auth"; sd_msg "Auth keyfile creation failed."; return 1; }
...
_guard_space() {
    ...
    sd_msg "$(printf '⚠  Less than 2 GiB free in the image.\nUse Other → Resize image to increase the size first.')"
    return 1
}

_resize_image() {
    [[ -z "$IMG_PATH" || ! -f "$IMG_PATH" ]] && { sd_msg "No image mounted."; return 1; }
    ...
    local new_gib_raw
    sd_input "$(printf 'Current: %s GB   Used: %s GB   Minimum: %s GB\n\nNew size in GB:' "$cur_gib" "$used_gib" "$min_gib")" || return 0
    new_gib_raw="${SD_MSG//[^0-9]/}"
    if [[ -z "$new_gib_raw" || "$new_gib_raw" -lt "$min_gib" ]]; then
        sd_msg "$(printf 'Invalid size. Must be a whole number ≥ %s GB.' "$min_gib")"; return 1
    fi
    ...
    sd_confirm "$confirm_msg" || return 0
...
RESIZESCRIPT
    ...
    if [[ -f "$ok_file" ]]; then
        ...
        sd_msg "Resize failed. Check that sudo commands succeeded."
