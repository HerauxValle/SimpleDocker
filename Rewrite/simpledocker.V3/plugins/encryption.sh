    ...
    while true; do
        ...
        case "$sc" in
            ...
            *"System Agnostic"*)
                if [[ "$_agnostic" == true ]]; then
                    ...
                    if [[ "$_has_passkeys" == false && "$_sa_vs_count" -eq 0 ]]; then
                        sd_msg "$(printf 'Cannot disable — no other unlock method exists.\nAdd a passkey or verify a system first.')"
                        continue
                    fi
                    sd_confirm "Disable System Agnostic? This image will no longer open on unknown machines." || continue
                    ...
                    [[ $_sa_rc -eq 0 ]] && sd_msg "System Agnostic disabled." || sd_msg "Failed."
                else
                    # Enable: add SD_DEFAULT_KEYWORD back to slot 1
                    if ! _enc_authkey_valid; then
                        sd_msg "$(printf 'Auth keyfile missing or invalid.\nUse Reset Auth Token first.')"
                        continue
                    fi
                    ...
                    [[ $_sa_erc -eq 0 ]] && sd_msg "System Agnostic enabled." || sd_msg "Failed."
                fi ;;

            *"Auto-Unlock"*)
                if [[ "$_auto" == true ]]; then
                    if [[ "$_has_passkeys" == false ]]; then
                        sd_msg "$(printf 'No passkeys exist.\nAdd a passkey first,\nthen disable Auto-Unlock.')"
                        continue
                    fi
                    sd_confirm "Disable Auto-Unlock? All verified system slots will be removed (cache kept)." || continue
                    clear
                    ...
                    rm -f "$_tf_dis"; clear
                    "$_dis_ok" && sd_msg "Auto-Unlock disabled." || sd_msg "Failed (some slots may remain)."
                else
                    if ! _enc_authkey_valid; then
                    ...
                    for _vsid in "${_vs_ids[@]}"; do
                        ...
                        if [[ -z "$_free_s" ]]; then
                            sd_msg "$(printf 'No free slots (slots %s-%s full).' "$SD_LUKS_KEY_SLOT_MIN" "$SD_LUKS_KEY_SLOT_MAX")"
                            _en_ok=false; break
                        fi
                    ...
                    if "$_en_ok"; then
                        [[ $_en_count -eq 0 ]] \
                            && sd_msg "No verified systems to restore. Use '+ Verify this system'." \
                            || sd_msg "$(printf 'Auto-Unlock enabled (%s system(s) restored).' "$_en_count")"
                    else
                        sd_msg "Partially failed — some systems may not have been restored."
                    fi
                fi ;;
            ...
            *"Reset Auth Token"*)
                ...
                [[ $_rrc -eq 0 ]] && sd_msg "Auth keyfile reset." || sd_msg "Failed — wrong passphrase?" ;;

            *"Verify this system"*)
                if _enc_is_verified; then
                    local _my_slot; _my_slot=$(_enc_vs_slot "$_vid")
                    if [[ -n "$_my_slot" && "$_my_slot" != "0" ]]; then
                        sd_msg "$(printf 'Already verified: %s (slot %s).' "$(cat /etc/hostname 2>/dev/null | tr -d "[:space:]" || printf "unknown")" "$_my_slot")"
                    else
                        sd_msg "$(printf 'System cached but Auto-Unlock is disabled.\nEnable Auto-Unlock to activate it.')"
                    fi
                    continue
                fi
                ...
                if [[ "$_auto" == true ]]; then
                    ...
                    if [[ $_vsrc -eq 0 ]]; then
                        ...
                        sd_msg "$(printf 'Verified: %s (slot %s, auto-unlock active).' "$(cat /etc/hostname 2>/dev/null | tr -d "[:space:]" || printf "unknown")" "$_free_vs")"
                    else
                        sd_msg "Failed to add key slot."
                    fi
                else
                    _enc_vs_write "$_vid" ""
                    sd_msg "$(printf 'Cached: %s (Auto-Unlock disabled — enable to activate).' "$(cat /etc/hostname 2>/dev/null | tr -d "[:space:]" || printf "unknown")")"
                fi ;;
                    ...
                    Unauthorize)
                        if [[ "$_sel_vsid" == "$_vid" && "$_auto" == true && "$_has_passkeys" == false ]]; then
                            ...
                            if [[ $_remaining_vs -eq 0 ]]; then
                                sd_msg "$(printf 'Cannot unauthorize — this is the only unlock method.\nAdd a passkey first.')"
                                continue
                            fi
                        fi
                        sd_sd_confirm "$(printf 'Unauthorize %s?' "$_sel_vshost")" || continue
                        clear
                        ...
                        "$_unauth_ok" && sd_msg "Unauthorize complete." || sd_msg "Failed to remove slot (cache removed)." ;;
                esac ;;
            ...
            *"[s:"*)
                ...
                case "$action" in
                    Rename)
                        sd_input "$(printf 'New name for "%s":' "$_cur_name")" || continue
                        local _nn="${SD_MSG}"; [[ -z "$_nn" ]] && continue
                    ...
                    Remove)
                        ...
                        if [[ "$_auto" == false && "$_pk_count" -le 1 ]]; then
                            sd_msg "$(printf 'Cannot remove — Auto-Unlock is disabled.\nKeep at least one passkey\nor re-enable Auto-Unlock first.')"
                            continue
                        fi
                        if [[ "$_auto" == true && "$_pk_count" -le 1 && "$_vs_active_count" -eq 0 ]]; then
                            sd_msg "$(printf 'Cannot remove — this is the only non-auto-unlock key.\nVerify a system or keep this key.')"
                            continue
                        fi
                        sd_sd_confirm "$(printf 'Remove key "%s"?' "$_cur_name")" || continue
                        clear
            ...
            *"Add Key"*)
                ...
                while [[ "$_param_done" == false ]]; do
                    ...
                    case "$_psel" in
                        ...
                        *"Cancel"*)   break ;;
                        *"name"*)
                            sd_input "$(printf 'Key name (blank = %s):' "$_rname")" || continue
                            _kname="${SD_MSG:-$_rname}" ;;
                        *"pbkdf"*)
                            local _pv; _pv=$(printf 'argon2id\nargon2i\npbkdf2\n' \
                                | fzf "${FZF_BASE[@]}" --header="pbkdf" 2>/dev/null | _trim_s)
                            [[ -n "$_pv" ]] && _pbkdf="$_pv" ;;
                        *"ram"*)
                            sd_input "RAM in KiB (e.g. 262144 = 256MB):" || continue
                            [[ "$SD_MSG" =~ ^[0-9]+$ ]] && _ram="$SD_MSG" ;;
                        *"threads"*)
                            sd_input "Threads (e.g. 4):" || continue
                            [[ "$SD_MSG" =~ ^[0-9]+$ ]] && _threads="$SD_MSG" ;;
                        *"iter-ms"*)
                            sd_input "Iteration time in ms (e.g. 1000):" || continue
                            [[ "$SD_MSG" =~ ^[0-9]+$ ]] && _iter="$SD_MSG" ;;
                        *"cipher"*)
                ...
                [[ "$_np1" != "$_np2" || -z "$_np1" ]] && { clear; sd_msg "Mismatch or empty."; continue; }
                ...
                [[ $_rc -ne 0 ]] && { sd_msg "Failed to add key."; continue; }
                ...
                sd_msg "$(printf 'Key "%s" added (slot %s).' "$_kname" "$_free_k")" ;;
        esac
    done
}
