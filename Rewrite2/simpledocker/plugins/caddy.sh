        ...
        case "$sc" in
            ...
            *"Caddy + mDNS"*)
                if $caddy_ok; then
                    ...
                    case "$REPLY" in
                        ...
                        "View log") sd_msg "$(cat "$(_proxy_caddy_log)" 2>/dev/null | tail -50 || echo "(no log)")" ;;
                        "View Caddyfile") sd_msg "$(cat "$(_proxy_caddyfile)" 2>/dev/null || echo "(no Caddyfile)")" ;;
                        "Reset proxy config")
                            sd_sd_confirm "$(printf '⚠  This will:\n  - Remove all custom rerouting URLs\n  - Reset all containers to default exposure (localhost)\n\nThe Caddyfile will be regenerated from scratch.\nContinue?')" || continue
                            ...
                            _proxy_start
                            sd_msg "Proxy config reset and restarted." ;;
                    esac
                else
            ...
            *"Running"*)
                if _proxy_running; then
                    _proxy_stop; _avahi_stop 2>/dev/null || true; sd_msg "Proxy stopped."
                else
                    if _proxy_start; then
                        _hostpkg_installed "avahi-utils" && _avahi_start
                        sd_msg "Proxy started."
                    else
                        ...
                        sd_msg "$(printf '⚠  Caddy failed to start.%s\n\nLog:\n%s' "$_extra" "$_caddy_log_tail")"
                    fi
                fi ;;
            *"Add URL"*)
                _load_containers false
                [[ ${#CT_IDS[@]} -eq 0 ]] && { sd_msg "No containers found."; continue; }
                local copts2=()
            ...
            *)
                ...
                case "$REPLY" in
                    ...
                    "Change URL")
                        sd_input "New URL:" || continue
                        local nu="${SD_MSG}"; nu="${nu#http://}"; nu="${nu#https://}"; nu="${nu%%/*}"
                        [[ -z "$nu" ]] && continue
                    ...
                    *"Toggle HTTPS"*)
                        ...
                        _proxy_running && { _proxy_stop; _proxy_start; } ;;
                    "Remove")
                        sd_confirm "Remove $matched?" || continue
